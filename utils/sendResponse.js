...
exports.sendTextResponse = (req, res, statusCode, data) => {
  ...
  return res.status(statusCode).send(json2PlainText(JSON.parse(JSON.stringify(data))));
};
exports.sendResponse = (req, res, statusCode, data, message) => {
  if (req.headers.accept == 'application/xml') this.sendXmlResponse(req, res, statusCode, data, message);
  else if (req.headers.accept == 'application/json') this.sendJsonResponse(req, res, statusCode, data, message);
  else if (req.headers.accept == 'text/plain') this.sendTextResponse(req, res, statusCode, data, message);
  //else if (req.headers.accept == 'text/html') this.sendTextHtmlResponse(req, res, statusCode, data, message);
